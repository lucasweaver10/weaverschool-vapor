...
<x-slot name="description">Create flashcards from a topic using the Weaver School's AI flashcard maker.</x-slot>
    <!-- Header Section -->
    <header class="flex justify-between items-center mb-10">
        <x-flashcards.sets.header.heading text="Create Flashcards from a Topic" />                
        <x-flashcards.sets.header.back-link text="Back to Sets" route="/flashcards/sets" />
    </header>        
        <div x-data="{ addVoice: true, howDoesThisWorkOpen: false }" class="bg-gray-900 min-h-screen flex flex-col items-center p-4">  
        ...
        <div x-cloak x-show="howDoesThisWorkOpen" class="fixed inset-0 bg-black bg-opacity-75 z-50 flex justify-center items-center">
            <div class="bg-gray-900 rounded-lg max-w-4xl mx-5 text-center relative p-4">
                ...
                <div class="w-full">
                    <iframe class="w-full h-96" src="https://www.youtube.com/embed/15eencgAf2E?si=a6sZQ00nHAkdnEFO" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                </div>
            </div>
        </div> 
        <form class="text-center form-control" method="POST" action="/flashcards/handle-from-topic">
        @csrf
           @if ($errors->any())
          <div class="mb-4">
          ...
          @endif
          <h1 class="text-gray-100 text-3xl sm:text-4xl font-bold mt-4 sm:mt-0 mb-12">What topic do you want to study?</h1>
            <div role="button" wire:ignore @click="howDoesThisWorkOpen = true" class="mb-12 text-sm text-white hover:text-teal-600 font-semibold">
                How does this work?
            ...
            <div class="bg-gray-800 mx-4 rounded-lg p-8 shadow-xl">
              <div class="mx-auto mb-5">
                  <textarea name="topic" id="topic" id="topic" rows="3" placeholder="'The days of the week in Spanish'" 
