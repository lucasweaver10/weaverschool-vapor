<x-layouts.app>
    <x-slot name="title">
        Cambridge Speaking Practice Tests | {{ config('app.name') }}
    </x-slot>
    <x-dashboard.index>
        <x-slot name="title">
            Cambridge Speaking Practice Tests
        </x-slot>
        <div>   
                ...
                <div class="bg-gray-800 hover:bg-gray-700 rounded-lg overflow-hidden shadow-lg my-4 w-full sm:w-80 md:w-96 mx-2 transition duration-300 ease-in-out transform hover:scale-105">
                    <div class="flex flex-col">
                        <div class="p-5 flex flex-col justify-between">
                            ...
                            <a href="{{ route('dashboard.exam-prep.cambridge.speaking.practice-tests.submit', ['locale' => session('locale', 'en'), 'id' => $test->id]) }}" class="px-4 py-2 mx-auto bg-teal-500 text-white font-semibold rounded-lg hover:bg-teal-600 transition ease-in-out duration-150">Take Test</a>
                        </div>
                    </div>
                </div>
            @endforeach
