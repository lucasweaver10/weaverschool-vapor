<x-layouts.app>
    <x-slot name="title">
        Cambridge Essay Checker | {{ config('app.name') }}
    </x-slot>
    <x-dashboard.index>
        <x-slot name="title">
            Submit an Essay
                        ...
                        @csrf
                            ...
                            <div class="mt-8">
                                ...
                                <select id="exam" name="exam" class="mt-2 mb-4 w-full block rounded-md text-gray-900 border-gray-300 focus:border-teal-300 focus:ring focus:ring-teal-200 focus:ring-opacity-50">
                                    <option value="C1 Advanced" {{ old('exam') === 'C1 Advanced' ? 'selected' : '' }}>C1 Advanced</option>
                                    <option value="C1 Business Higher" {{ old('exam') === 'C1 Business Higher' ? 'selected' : '' }}>C1 Business Higher</option>
                                    <option value="C2 Proficiency" {{ old('exam') === 'C2 Proficiency' ? 'selected' : '' }}>C2 Proficiency</option>
                                    <option value="B2 First" {{ old('exam') === 'IELTS General' ? 'selected' : '' }}>B2 First</option>                                  
                                </select>        
                            </div>
                            ...
                            <div class="mt-8">
                                ...
                                <select id="task" x-model="task" name="task" class="mt-2 mb-4 w-full block rounded-md text-gray-900 border-gray-300 focus:border-teal-300 focus:ring focus:ring-teal-200 focus:ring-opacity-50">
                                    <option value="Part One" {{ old('task') === 'Part One' ? 'selected' : '' }}>Part One</option>
                                    <option value="Part Two" {{ old('task') === 'Part Two' ? 'selected' : '' }}>Part Two</option>
                                </select>        
                            </div>
{{-- 
