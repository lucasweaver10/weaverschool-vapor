<x-layouts.app>
<x-slot name="title">
    Refer Your Friend | {{ config('app.name') }}
</x-slot>
<x-dashboard.index :user="$user">
    ...
    <main>        
            <h1 class="sr-only">Refer Your Friends</h1>
            <x-dashboard.profile.subnav /> 

            @unless($user->referral_code == null)
            <div class="bg-gray-300 dark:bg-gray-700 rounded-lg py-10 mt-12">
                <p class="lead text-center text-xl font-bold mb-4 text-gray-700 dark:text-gray-200">Your Referal Link</p>
                <div class="flex justify-center items-center w-full px-4">
                    <span id="referral-code" class="block max-w-full overflow-x-auto px-4 break-words cursor-pointer hover:bg-gray-200 hover:text-gray-900 rounded-lg p-2">{{ 'https://weaverschool.com/regsiter?refferalCode=' . $user->referral_code ?? '' }}</span>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const apiTokenSpan = document.getElementById('referral-code');
