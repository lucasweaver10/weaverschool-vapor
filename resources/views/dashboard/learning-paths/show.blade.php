    ...
    <div class="bg-gray-900 min-h-screen flex flex-col items-center p-4">
        <div class="w-full max-w-4xl mb-12">
            ...
            <x-flashcards.sets.header.back-link text="Back" route="{{ route('learning-paths.index', ['locale' => session('locale')])}}" />    
        </div>       
                ...
                x-transition:leave-end="opacity-0 transform scale-y-0">
                    @foreach ($path->vocabularyWords as $word)
                        <div id="word-{{ $word->id }}" class="bg-gray-800 hover:bg-gray-600 rounded-lg overflow-hidden shadow-lg shadow-gray-600/200 mb-6 p-8 transition duration-300 ease-in-out">
                            <h2 class="text-2xl font-bold text-white text-center mb-4">{{ $word->getTranslation('word', $path->pivot->target_locale) }} @unless(!$word->getTranslation('romanized_word', $path->pivot->target_locale))({{ $word->getTranslation('romanized_word', $path->pivot->target_locale) }})@endunless                           
                            ...
                            @unless(!$word->getMedia('images')->first())
                                @if($loop->first)
                                    <img class="mx-auto my-8 w-32 sm:w-48 md:w-64 lg:w-80 h-auto rounded-md" src="{{ $word->getMedia('images')->first()->getUrl('small') ?? $word->image_url }}" alt="{{ $word->getTranslation('word', $path->pivot->target_locale) }}">
                                @else
                                <img class="lazy mx-auto my-8 w-32 sm:w-48 md:w-64 lg:w-80 h-auto rounded-md" data-src="{{ $word->getMedia('images')->first()->getUrl('small') ?? $word->image_url }}" alt="{{ $word->getTranslation('word', $path->pivot->target_locale) }}">
                                @endif
                            @endunless
                            <div class="flex justify-center">                    
                                <a href="{{ route('learning-paths.vocabulary.show', ['locale' => session('locale'), 'learningPathId' => $path->id, 'vocabularyWordId' => $word->id]) }}" 
                ...
                <div x-cloak x-show="open" 
                    ...
                    @foreach ($path->keyPhrases as $phrase)
                        <div id="phrase-{{ $phrase->id }}" class="bg-gray-800 hover:bg-gray-600 rounded-lg overflow-hidden shadow-lg shadow-gray-800/600 mb-6 p-8">
                            <h2 class="text-2xl font-bold text-white text-center mb-4">{{ $phrase->getTranslation('phrase',  $path->pivot->target_locale) }} @unless(!$phrase->getTranslation('romanized_phrase',  $path->pivot->target_locale))({{ $phrase->getTranslation('romanized_phrase',  $path->pivot->target_locale) }})@endunless
                            ...
                            @unless(!$phrase->getMedia('images')->first())
                                @if($loop->first)
                                    <img class="mx-auto my-8 w-32 sm:w-48 md:w-64 lg:w-80 h-auto rounded-md" src="{{ $phrase->getMedia('images')->first()->getUrl('small') ?? $phrase->image_url }}" alt="{{ $phrase->getTranslation('phrase', $path->pivot->target_locale) }}">
                                @else
                                    <img class="lazy mx-auto my-8 w-32 sm:w-48 md:w-64 lg:w-80 h-auto rounded-md" data-src="{{ $phrase->getMedia('images')->first()->getUrl('small') ?? $phrase->image_url }}" alt="{{ $phrase->getTranslation('phrase', $path->pivot->target_locale) }}">
                                @endif
                            @endunless                        
                            <div class="flex justify-center">
                            
                                <a href="{{ route('learning-paths.phrases.show', ['locale' => session('locale'), 'learningPathId' => $path->id, 'phraseId' => $phrase->id]) }}" 
                                class="mt-4 bg-teal-500 hover:bg-teal-400 text-white text-xl font-bold py-1 px-3 rounded focus:outline-none focus:shadow-outline transform transition hover:scale-105 duration-300 ease-in-out">
                                    Learn
                                </a>
                            </div>
                ...
                <div x-cloak x-show="open" 
                    ...
                    @foreach ($path->culturalNotes as $note)
                        <div id="note-{{ $note->id }}" class="bg-gray-800 rounded-lg overflow-hidden shadow-lg shadow-gray-800/600 mb-6 p-8">
                            <h2 class="text-2xl font-bold text-white text-center mb-4">{{ $note->getTranslation('title',  $path->pivot->native_locale) }}</h2>
                            {{-- <div class="text-gray-200 mt-2">{!! Str::limit(Str::of($note->getTranslation('content',  $path->pivot->native_locale))->markdown(), 150, '...') !!}</div> --}}
                            <div class="flex justify-center">
                                <a href="{{ route('learning-paths.cultural-notes.show', ['locale' => session('locale'), 'learningPathId' => $path->id, 'noteId' => $note->id]) }}" class="mt-4 bg-teal-500 hover:bg-teal-400 text-white text-xl font-bold py-1 px-3 rounded focus:outline-none focus:shadow-outline transform transition hover:scale-105 duration-300 ease-in-out">
                                    Learn
                                </a>
                            </div>
                        </div>
