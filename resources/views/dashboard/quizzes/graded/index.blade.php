<x-layouts.app>
    <x-dashboard.index :user="$user">
        ...
        @if(count($user->quizAssignments) < 1)
            <div>
                No open quizzes.
            </div>
        @endif
        @foreach($user->quizAssignments->whereNotNull('completed_at') as $quizAssignment)
                                ...
                                <div class="min-w-0 flex-1 px-4 md:grid md:grid-cols-3 md:gap-4">
                                    <div>
                                        ...
                                        <div class="text-sm">
                                            Completed
                                            <time
                                                datetime="2020-01-07">{{ $quizAssignment->completed_at->toFormattedDateString() }}</time>
                                        </div>
                                    </div>
                                    <div>
                                        <div class="text-md font-medium">{{ $quizAssignment->score }} / 10</div>
...
{{--                                    <a href="/{{ session('locale') ?? 'en' }}/dashboard/quizzes/graded/{{ $quizAssignment->id }}" class="block hover:bg-gray-50 unstyled">--}}
