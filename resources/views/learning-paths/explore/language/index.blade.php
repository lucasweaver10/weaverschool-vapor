<x-layouts.app>      
    <x-slot name="title">
        Learning Paths for learning {{ $paths->first()->targetLanguage->name }}
    </x-slot>
    <x-slot name="description">
        Explore our available Learning Paths for learning the {{ $paths->first()->targetLanguage->name }} language. Choose a topic and start learning vocabulary, phrases, and grammar in minutes.
    </x-slot>
<x-layouts.guest>
...
<div class="dark:bg-gray-900 pt-20 pb-18 px-5 sm:px-0 text-center lg:pt-16">
    <div class="w-full flex -mt-12">
        <div class="justify-start ml-32">
            <x-flashcards.sets.header.back-link text="Back to all" route="{{ route('learning-paths.explore.index', ['locale' => session('locale', 'en')])}}" />
        </div>
    </div>
    <h1 class="mx-auto max-w-4xl font-display text-5xl font-medium tracking-tight text-slate-900 dark:text-gray-100 sm:text-7xl">
        ...
        <span class="relative whitespace-nowrap text-teal-500 dark:text-teal-300">
            ...
            <span class="relative">{{ $paths->first()->targetLanguage->name }}</span>
        </span>
        
    </h1>
    <p class="mx-auto sm:px-32 mt-10 max-w-4xl text-2xl tracking-tight text-slate-700 dark:text-slate-200">
        Learn to speak about any topic in the {{ $paths->first()->targetLanguage->name }} language you want in just two weeks. Click the
         study button to either follow an existing path or customize it for your native language.
    </p>    
</div>
<!-- End hero -->
 ...
 <!-- Loop through existing learning paths -->
    ...
    <div class="relative bg-gray-800 dark:bg-gray-800 hover:bg-gray-700 rounded-lg overflow-hidden shadow-lg my-10 w-full max-w-4xl mx-auto transition duration-300 ease-in-out transform hover:scale-105">                    
        <div class="flex flex-col sm:flex-row">
            ...
            <div class="flex-1 p-5 flex flex-col justify-between">
                ...
                <div class="flex justify-between mb-2"> <!-- Align button to the end of the flex container -->                    
                    <button role="button" @click="showStudy{{ $path->id }}Modal = true" class="px-3 py-2 rounded-lg bg-teal-500 text-white font-semibold hover:bg-teal-600 transition ease-in-out duration-150">Study</button>
                    <a href="{{ route('learning-paths.explore.show', ['locale' => session('locale', 'en'), 'languageSlug' => $path->targetLanguage->slug, 'pathSlug' => $path->slug ])}}" class="px-3 py-2 rounded-lg bg-violet-500 text-white font-semibold hover:bg-violet-600 transition ease-in-out duration-150">Preview</a>
                </div>
            </div>
        </div>
    </div>
