<div class="mt-20 mx-auto">
    <div class="text-4xl sm:text-5xl font-bold text-white text-center mt-16">
        @if($course->plans->first()->discounted_total_price)@lang('currency.symbol'){{ $course->plans->first()->discounted_total_price }}@else @lang('currency.symbol'){{ $course->plans->first()->total_price }}@endif
        @if($course->plans->first()->discounted_total_price)<strike class="text-blue-400"><span class="text-white">@lang('currency.symbol'){{ $course->plans->first()->total_price }}</span></strike>@endif
    </div>

    <div class="text-center mt-10">
        @guest
            <a x-bind:href="'/register?course_id=' + {{ $course->id }} + '&plan_id=' + plan"
               class="bg-teal-400 hover:bg-teal-300 rounded-lg py-2 px-5 text-3xl sm:text-5xl text-white font-bold w-screen">
                Start for FREE
            </a>
        @else
