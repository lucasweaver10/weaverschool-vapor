...
@php $path = App\Models\LearningPath::with('vocabularyWords')->find(3); @endphp
        ...
        <div class="flex flex-col sm:flex-row"> <!-- Set a fixed height for the flex container -->
            ...
            <div x-data="{ activeWord: 0, words: {{ $path->vocabularyWords->take(5)->pluck('id') }}, interval: null }"
                ...
                <div class="mx-auto">
                        ...
                        @foreach ($path->vocabularyWords->take(5) as $index => $word)
                            <div x-cloak x-show="words[activeWord] === {{ $word->id }}"
                                ...
                                <div class="font-bold flex items-start text-gray-100">
                                    {{ $word->getTranslation('word', 'zh-CN') }}
                                    @unless(!$word->getTranslation('romanized_word', 'zh-CN'))
                                        ({{ $word->getTranslation('romanized_word', 'zh-CN') }})
                                    @endunless
                                    @unless(!$word->getWordAudioUrl('zh-CN', 'FEMALE'))
                                        <audio id="wordAudio{{ $word->id }}" src="{{ $word->getWordAudioUrl('zh-CN', 'FEMALE') }}" preload="true"></audio>
                                        <button @click="$refs['wordAudio{{ $word->id }}'].play()" class="">
                    ...
                    <a href="{{ route('learning-paths.info')}}" 
