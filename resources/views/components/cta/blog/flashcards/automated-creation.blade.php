...
@php $set = App\Models\FlashcardSet::with('flashcards')->find(457); @endphp
    ...
    <div class="bg-gray-700 hover:bg-gray-600 rounded-lg shadow-lg w-full max-w-4xl mx-auto">
        <div class="flex flex-col sm:flex-row"> <!-- Set a fixed height for the flex container -->
            ...
            <div class="w-full sm:w-1/3">
                <img src="{{ $set->flashcards->first()->image_url ?? asset('images/weaver_school_learning_path_fallback_image.webp') }}" 
                alt="{{ $set->title }}" 
                class="object-cover w-full h-full sm:rounded-l-lg mb-2 sm:mb-0">
            </div>
            <!-- Content on the right -->
            <div x-data="{ activeWord: 0, words: {{ $set->flashcards->take(5)->pluck('id') }}, interval: null }"
                ...
                <div class="mx-auto">
                    <div class="text-3xl font-bold text-teal-400 sm:mb-4 sm:px-4">{{ $set->title }}</div>
                    <div class="relative overflow-hidden mb-4" style="height: 200px;">
                        @foreach ($set->flashcards->take(5) as $index => $flashcard)
                            <div x-cloak x-show="words[activeWord] === {{ $flashcard->id }}"
                                ...
                                <div class="font-bold flex items-start text-gray-100">
                                    {{ $flashcard->term }}
                                    @unless(!$flashcard->romanized_term)
                                        ({{ $flashcard->romanized_term }})
                                    @endunless
                                    @unless(!$flashcard->audio_url)
                                        <audio id="wordAudio{{ $flashcard->id }}" src="{{ $flashcard->audio_url }}" preload="true"></audio>                                                                                
                                ...
                                <div class="">
                                    <img src="{{ $flashcard->image_url }}" alt="" class="ml-2 h-24 w-auto rounded">
                                </div>
                            </div>
                        @endforeach
                    </div>
                    <a href="{{ route('flashcards.info', ['locale' => session('locale', 'en')])}}" 
