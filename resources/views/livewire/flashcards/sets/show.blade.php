                        ...
                        <div wire:key="{{ $flashcard->id }}" class="bg-gray-800 hover:bg-gray-700 flex flex-col rounded-lg shadow-lg overflow-hidden transition duration-300 ease-in-out transform hover:scale-105">
                            ...
                            <div class="p-5">
                                <div class="flex justify-between items-center">
                                    ...
                                    <div class="flex">
                                        <livewire:flashcards.audio.create :flashcardId="$flashcard->id" :key="'audio-create-' . $flashcard->id" />                        
                                        <livewire:flashcards.images.create :flashcardId="$flashcard->id" :key="'image-create-' . $flashcard->id"/>                                
                                    </div>
                                </div>
                            ...
                            <div class="flex px-5 py-3">
                                <a href="/flashcards/{{ $this->flashcardSet->id }}/{{ $flashcard->id }}/edit" class="py-2 text-teal-400 hover:text-teal-900 font-bold text-sm mt-auto">Edit</a>
                                <form class="ml-auto" action="/flashcards/{{ $flashcard->id }}/destroy" method="POST" onsubmit="return confirm('Are you sure you want to delete this flashcard?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="py-2 text-red-400 hover:text-red-600 font-bold text-sm">Delete</button>
                                </form>
                            </div>
                            </div>
                        </div>
