<div x-data="{ mobileActionButtonOpen: false }">
    <header class="flex flex-col md:flex-row justify-between items-center w-full">
        <x-flashcards.sets.header.heading text="Your Flashcard Sets" />
        <div class="grid grid-cols-3 sm:grid-cols-2 mb-8 sm:mb-0 sm:flex gap-x-4 items-center">
            ...
            <x-flashcards.sets.header.secondary-button text="Explore" :route="route('flashcards.sets.explore.index', ['locale' => session('locale', 'en')])" />
            <x-flashcards.sets.header.button text="Create" :route="route('flashcards.sets.create')" />
        </div>
    </header>
    ...
    <div class="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div wire:key="{{ $set->id }}" class="bg-gray-800 hover:bg-gray-600 rounded-lg overflow-hidden shadow-lg shadow-gray-600/200 flex flex-col h-full transition duration-300 ease-in-out transform hover:scale-105">
            ...
            <div class="flex justify-between items-center p-4">
                <!-- Edit Button -->
                <a role="button" href="/flashcards/sets/{{ $set->id }}" class="py-2 text-teal-300 hover:text-teal-100 font-bold text-sm rounded-lg">
                    Edit
                </a>
