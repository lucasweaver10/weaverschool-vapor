<div x-data="{ status: @entangle('status').live }">
   ...
   <div x-show="status == 'open'">
         @foreach ($teacher->assignments->where('status', 'open') as $assignment)
            <div>
    ...
    <div x-show="status == 'completed'">
        @foreach ($teacher->assignments->where('status', 'completed') as $assignment)
            <div>
                <livewire:myteacher.courses.assignments.show :assignment="$assignment" :wire:key="$assignment->id" />
            </div>
        @endforeach
    ...
    <div x-show="status == 'graded'">
        @foreach ($teacher->assignments->where('status', 'graded') as $assignment)
            <div>
                <livewire:myteacher.courses.assignments.show :assignment="$assignment" :wire:key="$assignment->id" />
            </div>
        @endforeach
