    ...
    <div x-data="{editMode: false}" class="p-20">
                <div class="mb-4">
                    ...
                    <div x-show="editMode === false">
                        <!-- Livewire component for creating written_examples -->
                        <livewire:admin.lessons.written-examples.index :lesson="$lesson" class="mb-4"/>
                    </div>
                                        ...
                                        <table class="min-w-full divide-y divide-gray-300">
                                            ...
                                            @if($lesson->writtenExamples)
                                                @foreach($lesson->writtenExamples as $example)                                                
                                                    <livewire:admin.lessons.written-examples.edit :lesson="$lesson" :example="$example" wire:key="{{ $index }}"/>                                            
                                                @endforeach                                                
                                            @endif
                                            </tbody>
                                        </table>
