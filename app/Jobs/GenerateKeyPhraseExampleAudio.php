...
class GenerateKeyPhraseExampleAudio implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $audioUrls = $phrase->example_audio_urls ? json_decode($phrase->example_audio_urls, true) : [];
        ...
        if (!isset($audioUrls[$this->targetLocale][$voice->ssml_gender])) {
            ...
            $text = $phrase->getTranslation('example', $this->targetLocale, false);
            $audioContent = $audioService->generateAudioFromText($text, $voice->voice_locale, $voice->voice_name, $voice->ssml_gender);
            $filename = 'key-phrase-example-' . $phrase->id . '-' . $voice->ssml_gender . '-' . $this->targetLocale . '-' . uniqid() . '.mp3';            
            $path = '/audio/key-phrases/examples/' . $filename;
            ...
            $phrase->example_audio_urls = json_encode($audioUrls, JSON_UNESCAPED_SLASHES);
